...
function drawScene(timestamp) {
    ...
    const velocidade1 = 200;
    const velocidade2 = 500;
    const velocidade3 = 100;
    const scaleMatrix = createScaleMatrix(parseFloat(escala.value), parseFloat(escala.value));
    const rotationMatrixZ = createRotationMatrixZ(variacao*velocidade1 * Math.PI / 180);
    const rotationMatrixZ2 = createRotationMatrixZ(variacao2*velocidade2 * Math.PI / 180);
    const rotationMatrixZ3 = createRotationMatrixZ(variacao3*velocidade3 * Math.PI / 180);
    ...
    const transformMatrix2 = multiplyMatrices(orthoMatrix, multiplyMatrices(rotationMatrixZ2, multiplyMatrices(translationMatrix2, scaleMatrix)));
    const transformMatrix3 = multiplyMatrices(orthoMatrix, multiplyMatrices(rotationMatrixZ3, multiplyMatrices(translationMatrix3, scaleMatrix)));
