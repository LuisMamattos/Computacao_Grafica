...
uniform float u_parametroShadow;
uniform float u_shadowSamples;

out vec4 outColor;

float calculateShadow(vec3 projectedTexcoord) {
  float currentDepth = projectedTexcoord.z + u_bias;
  float shadow = 0.0;

  // Kernel offsets for PCF
  vec2 texelSize = 1.0 / vec2(textureSize(u_projectedTexture, 0));
  for(float x = -u_parametroShadow; x <= u_parametroShadow; x += u_parametroShadow) {
      for(float y = -u_parametroShadow; y <= u_parametroShadow; y += u_parametroShadow) {
          float depth = texture(u_projectedTexture, projectedTexcoord.xy + vec2(x, y) * texelSize).r;
          if(depth < currentDepth) {
              shadow += 1.0;
          }
      }
  }
  shadow /= u_shadowSamples * u_shadowSamples;
  return shadow;
}
...
void main() {
    ...
    float shadowLight = (inRange) ? calculateShadow(projectedTexcoord) : 1.0;

    vec4 texColor = texture(u_texture, v_texcoord) * u_colorMult;
    outColor = vec4(
        texColor.rgb * light * (1.0 - shadowLight) +
        specular * (1.0 - shadowLight),
        texColor.a);
}
`;
...
function main() {
  ...
  webglLessonsUI.setupUI(document.querySelector('#ui'), settings, [
    { type: 'slider', key: 'parametroShadow', min: 0.1, max: 10, change: render, precision: 2, step: 0.1 },
    { type: 'slider', key: 'shadowSamples', min: 3, max: 4, change: render, precision: 2, step: 0.1 },
  ...
  function drawScene(
    ...
    twgl.setUniforms(programInfo, {
      ...
      u_parametroShadow: settings.parametroShadow,
      u_shadowSamples: settings.shadowSamples,
    });
